...
class TestCLIErrorPaths:
    ...
    def test_migrate_subprocess_error(self, tmp_path, monkeypatch):
        """Test migrate command with subprocess error."""
        monkeypatch.setenv("PRODUCTHUNT_TOKEN", "test_token_12345678")

        with patch("subprocess.run") as mock_run:
            mock_run.side_effect = subprocess.CalledProcessError(1, "alembic", stderr="Migration failed")

            result = runner.invoke(app, ["migrate", "test migration"])

            assert result.exit_code == 1
            assert "Migration creation failed" in result.stdout

    def test_migrate_general_exception(self, tmp_path, monkeypatch):
        """Test migrate command with general exception."""
        monkeypatch.setenv("PRODUCTHUNT_TOKEN", "test_token_12345678")
    ...
    def test_upgrade_subprocess_error(self, tmp_path, monkeypatch):
        """Test upgrade command with subprocess error."""
        monkeypatch.setenv("PRODUCTHUNT_TOKEN", "test_token_12345678")

        with patch("subprocess.run") as mock_run:
            mock_run.side_effect = subprocess.CalledProcessError(1, "alembic", stderr="Upgrade failed")
    ...
    def test_upgrade_general_exception(self, tmp_path, monkeypatch):
        """Test upgrade command with general exception."""
        monkeypatch.setenv("PRODUCTHUNT_TOKEN", "test_token_12345678")
    ...
    def test_downgrade_subprocess_error(self, tmp_path, monkeypatch):
        """Test downgrade command with subprocess error."""
        monkeypatch.setenv("PRODUCTHUNT_TOKEN", "test_token_12345678")

        with patch("subprocess.run") as mock_run:
            mock_run.side_effect = subprocess.CalledProcessError(1, "alembic", stderr="Downgrade failed")

            result = runner.invoke(app, ["downgrade", "-1"])

            assert result.exit_code == 1
            assert "Downgrade failed" in result.stdout

    def test_downgrade_general_exception(self, tmp_path, monkeypatch):
        """Test downgrade command with general exception."""
        monkeypatch.setenv("PRODUCTHUNT_TOKEN", "test_token_12345678")
    ...
    def test_migration_history_subprocess_error(self, tmp_path, monkeypatch):
        """Test migration-history command with subprocess error."""
        monkeypatch.setenv("PRODUCTHUNT_TOKEN", "test_token_12345678")

        with patch("subprocess.run") as mock_run:
            mock_run.side_effect = subprocess.CalledProcessError(1, "alembic", stderr="History failed")

            result = runner.invoke(app, ["migration-history"])

            assert result.exit_code == 1
            assert ("Failed to retrieve migration history" in result.stdout or 
                    "Failed to get migration history" in result.stdout)

    def test_migration_history_general_exception(self, tmp_path, monkeypatch):
        """Test migration-history command with general exception."""
        monkeypatch.setenv("PRODUCTHUNT_TOKEN", "test_token_12345678")
    ...
    def test_migrate_with_stdout_output(self, tmp_path, monkeypatch):
        """Test migrate command prints stdout output."""
        monkeypatch.setenv("PRODUCTHUNT_TOKEN", "test_token_12345678")
        ...
        with patch("subprocess.run") as mock_run:
            ...
            assert result.exit_code == 0
            assert "Generated migration file" in result.stdout or "Migration created successfully" in result.stdout

    def test_upgrade_with_stdout_output(self, tmp_path, monkeypatch):
        """Test upgrade command prints stdout output."""
        monkeypatch.setenv("PRODUCTHUNT_TOKEN", "test_token_12345678")
    ...
    def test_subprocess_error_with_stderr(self, tmp_path, monkeypatch):
        """Test subprocess error displays stderr."""
        monkeypatch.setenv("PRODUCTHUNT_TOKEN", "test_token_12345678")
        ...
        with patch("subprocess.run") as mock_run:
            ...
            assert result.exit_code == 1
            # Should display error message in output
